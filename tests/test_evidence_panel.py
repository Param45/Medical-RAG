...
@patch("streamlit.code")
@patch("streamlit.markdown")
def test_render_evidence_popover_content(mock_md, mock_code, mock_img, mock_tabs, mock_get_ev):
    """Verify popover content renders metadata, code, and tabs."""
    from app import render_evidence_popover_content
    ...
    render_evidence_popover_content(["patient_a__report_1__page_1__chunk_0"])

    mock_get_ev.assert_called_once()
    mock_code.assert_called_once_with("Infiltrating duct carcinoma, Grade III", language=None)


@patch("app.render_evidence_popover_content")
@patch("streamlit.popover")
@patch("streamlit.columns")
@patch("streamlit.markdown")
def test_render_answer_with_evidence_popovers(mock_md, mock_cols, mock_popover, mock_render_pop):
    """Verify render_answer_with_evidence_popovers creates popovers on cited lines."""
    from app import render_answer_with_evidence_popovers
    ...
    pop_cm = MagicMock()
    pop_cm.__enter__.return_value = pop_cm
    pop_cm.__exit__.return_value = None
    mock_popover.return_value = pop_cm

    text = (
        "Based on your records:\n\n"
        "- Metastasis [patient_a__report_0013__page_14__chunk_0]\n"
        "- Clear scan findings"
    )

    render_answer_with_evidence_popovers(text)

    # Cited line should trigger columns and popover
    mock_cols.assert_called_once()
    mock_popover.assert_called_once()
    mock_render_pop.assert_called_once_with(["patient_a__report_0013__page_14__chunk_0"])
